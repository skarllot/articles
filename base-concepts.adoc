...
independente de transmitir informações usando como base objetos {JSON}. Essas
informações são confiáveis pois são assinadas, conforme a técnica que abordamos
na seção anterior.
...
Como as partes anteriores a assinatura também é codificada utilizando o
algoritmo {Base64Url}, antes de compor o JWT.
